...
class AkinatorButton(Button):
    def __init__(self,akinator_obj,author,ques,style,label):
        super().__init__(label=label,style=style)
        self.aki = akinator_obj
        self.author = author
        self.q = ques
    ...
    async def callback(self,interaction:discord.Interaction):
        ...
        if interaction.user != self.author:
            await a.send("Get out of here",ephemeral=True)
        else:
            if self.label == "Yes":
                if self.aki.progression <=80:
                    self.q['q'] = await self.aki.answer("yes")
                    embed= discord.Embed(title="Akinator",description=self.q["q"],colour=discord.Color.random())
                    await interaction.response.edit_message(embed=embed)
                else:
                    await self.aki.win()
                    self.view.clear_items()
                    correct = discord.Embed(title = f"It's {self.aki.first_guess['name']}",description=f"({self.aki.first_guess['description']})! Was I correct?\n",colour=discord.Color.random())
                    correct.set_image(url=self.aki.first_guess['absolute_picture_path'])
                    await interaction.response.edit_message(view=self.view)
                    await a.send(embed=correct)


            elif self.label == "No":
                if self.aki.progression <=80:
                    self.q['q'] = await self.aki.answer("no")
                    embed= discord.Embed(title="Akinator",description=self.q["q"],colour=discord.Color.random())
                    await interaction.response.edit_message(embed=embed)
                else:
                    await self.aki.win()
            ...
            elif self.label == "I Don't Know":
                if self.aki.progression <=80:
                    self.q['q'] = await self.aki.answer("i")
                    embed= discord.Embed(title="Akinator",description=self.q["q"],colour=discord.Color.random())
                    await interaction.response.edit_message(embed=embed)
                else:
                    await self.aki.win()
            ...
            elif self.label == "Probably":
                if self.aki.progression <=80:
                    self.q['q'] = await self.aki.answer("p")
                    embed= discord.Embed(title="Akinator",description=self.q["q"],colour=discord.Color.random())
                    await interaction.response.edit_message(embed=embed)
                else:
                    await self.aki.win()
            ...
            elif self.label == "Probably Not":
                if self.aki.progression <=80:
                    self.q['q'] = await self.aki.answer("pn")
                    embed= discord.Embed(title="Akinator",description=self.q["q"],colour=discord.Color.random())
                    await interaction.response.edit_message(embed=embed)
                else:
                    await self.aki.win()
